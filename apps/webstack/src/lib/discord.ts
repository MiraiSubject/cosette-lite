...
export async function getGuildMember(id: string): Promise<{
	...
	try {
		const response = await fetch(
			`https://discord.com/api/v10/guilds/${config.discord.guildId}/members/${id}`,
			{
				method: 'GET',
				headers: {
		...
		switch (response.status) {
			...
			default: {
				...
				logger.error(
					`Error checking if user ${id} exists in guild: ${response.status}: ${response.statusText} ${'code' in errRes ? errRes.code : ''} ${'message' in errRes ? errRes.message : ''}`
				);
...
export async function joinDiscordServer(
	...
	try {
		...
		switch (response.status) {
			...
			default:
				logger.error(
					`Error joining ${userId} to server: ${response.status}: ${response.statusText} ${errRes.code} ${errRes.message}`
				);
	...
	try {
		...
		switch (response.status) {
			...
			default: {
				...
				logger.error(
					`Error modifying guild member ${userId}: ${response.status}: ${response.statusText} ${errRes.code} ${errRes.message}`
				);
				return { result: BotResult.Error, error: errRes };
			}
		}
